...
import { useCreatePIE, useUpdatePIE, useGetPIE, useCreateServiceLine, useUpdateServiceLine, useGetServiceLine, useCreateProjectStatus, useUpdateProjectStatuses, useGetProjectStatuses } from '../../../hooks/useRefBooks';
...
export function ServiceLineDialog({
    ...
    const createServiceLine = useCreateServiceLine();
    const updateServiceLine = useUpdateServiceLine();
    const { refetch: refetchServiceLine } = useGetServiceLine();
    ...
    const handleSubmit = async () => {
        if (!formData.name.trim()) {
            toast.error('Please enter a service line name');
            return;
        }
        ...
        try {
            if (editingPie) {
                await updateServiceLine.mutateAsync({
                    id: editingPie.id,
                    name: formData.name,
                });
                toast.success('service line updated');
            } else {
                await createServiceLine.mutateAsync({
                    name: formData.name,
                });
                toast.success('service line created');
            }

            await refetchServiceLine();
            onSuccess?.();
            handleClose();
        } catch (error: any) {
            console.error(error);
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            ...
            >
                <DialogHeader>
                    <DialogTitle>
                        {editingPie ? 'Edit service line' : 'Add service line'}
                    </DialogTitle>
...
export default ServiceLineDialog;