...
const SimpleDepartmentsTables = () => {
  ...
  const { data: task_types, isLoading, error, refetch: refetchTaskTypes } = useGetTaskTypes();
  const { mutate: editTask } = useEditTask();
  const { mutate: deleteTask } = useDeleteTask();
  const { mutate: createTask } = useCreateTask();

  // Хуки для типов задач
  ...
  const handleAddTask = () => {
    ...
    createTask(
      { name: newTask.name.trim(), task_type: selectedTaskType },
      {
        onSuccess: () => {
  ...
  const handleSaveEditTask = () => {
    ...
    editTask(
      { task_id: editingTask.id.toString(), body: { name: editTaskData.name.trim() } },
      {
        onSuccess: () => {
  ...
  const handleDeleteTaskTypeClick = (taskType: TaskType) => {
  ...
  const handleSaveTaskType = () => {
    ...
    if (editingTaskType) {
      updateTaskType(
        { id: editingTaskType.id, name: newTaskTypeName.trim() },
        {
          onSuccess: () => {
  ...
  const handleConfirmDeleteTaskType = () => {
    if (!taskTypeToDelete) return;
    deleteTaskType(taskTypeToDelete.id.toString(), {
      onSuccess: () => {
  ...
  return (
    <div className="container mx-auto p-6 space-y-8">
        ...
        <DialogContent className="sm:max-w-[500px]">
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              <Label htmlFor="taskTypeName">Type Name *</Label>
              <Input
                id="taskTypeName"
          ...
          <DialogFooter>
            ...
            <Button
              ...
              style={{ backgroundColor: "#111827", color: "white" }}
            >
              {editingTaskType ? "Save Changes" : "Create Type"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>

      {/* Диалог удаления типа задачи */}
        ...
        <DialogContent className="sm:max-w-[450px]">
          ...
          <div className="space-y-4 py-4">
            <div className="p-3 bg-red-50 rounded-lg">
              <p className="font-medium text-gray-900">{taskTypeToDelete?.name}</p>
            </div>
            <div className="text-sm text-gray-600">
              ...
              <p className="mt-1 text-red-600">
                This will also delete all tasks associated with this type. This action cannot be undone.
              </p>
            </div>
          </div>
              ...
              <div className="rounded-lg border border-l-4 border-l-blue-500 bg-blue-50">
                <div className="p-4 border-b bg-white/50">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h2 className="text-lg font-bold text-gray-900">{taskType.name}</h2>
                ...
                {typeTasks.length > 0 ? (
                  ...
                  <div className="p-8 text-center text-gray-500">
                    ...
                    <p className="text-sm mt-2">Add tasks using the main "Add Task" button</p>
                  </div>
                )}
      ...
      {/* Пустое состояние */}
      {(!store_tasks || store_tasks.length === 0 || visibleTaskTypes.length === 0) && (
        <Card className="text-center py-12 mt-8">
          <CardContent>
            ...
            <h3 className="text-xl font-semibold text-gray-700 mb-2">
              {!store_tasks || store_tasks.length === 0 ? "No Tasks Yet" : "No matching tasks"}
            </h3>
