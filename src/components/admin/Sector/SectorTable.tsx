...
import { useGetPIE, useDeletePIE, useGetSector, useDeleteSector } from '../../../hooks/useRefBooks';
import { toast } from 'sonner';
import SectorDialog from '../Sector/SectorDialog';
...
export function SectorTable({
    ...
    const { data: sectors, isLoading, error, refetch } = useGetSector();
    const deleteSectors = useDeleteSector();
    ...
    const handleConfirmDelete = async () => {
        if (pieToDelete) {
            try {
                await deleteSectors.mutateAsync(pieToDelete.id);
                await refetch();
    ...
    const safeData = Array.isArray(sectors) ? sectors : [];
            ...
            <div className="space-y-4" >
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground">Manage all sector in the collection</p>
                    </div>
                ...
                <div className="rounded-md border p-8 text-center">
                    <p className="text-red-500">Error loading sector: {error.message}</p>
    ...
    return (
        <>
            <div className="space-y-4">
                <div className="flex items-center justify-between">
                    <div>
                        <h2 className="text-xl font-bold tracking-tight">sector</h2>
                        <p className="text-sm text-muted-foreground">Manage all sector in the collection</p>
                    </div>
                    <Button onClick={handleAddClick} size="sm">
                        <Plus className="w-4 h-4 mr-2" />
                                ...
                                <TableRow>
                                    <TableCell colSpan={3} className="text-center text-slate-500 py-8">
                                        <div className="flex flex-col items-center gap-2">
                                            ...
                                            <span>No Sectors found. Click "Add Sector" to create your first sector.</span>
                                        </div>
                                    </TableCell>
                                </TableRow>
                            ) : (
                            ...
                            <AlertDialogHeader>
                                ...
                                <AlertDialogDescription asChild>
                                    <div>
                                        {pieToDelete && (
                                            <>
                                                You are about to delete sector #{pieToDelete.id} - {pieToDelete.name}.

                                                <div className="mt-3 text-sm text-amber-600">
                                                    This action cannot be undone. All sector data will be permanently removed.
                                                </div>

                                                <div className="mt-2 p-3 bg-red-50 rounded-md">
                                                    <div className="text-sm font-medium text-red-700">
                                                        ⚠️ Warning: Deleting this sector will remove it from all associated records.
                                                    </div>
                                                </div>
                                            </>
                                        )}
