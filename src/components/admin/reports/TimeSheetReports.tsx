...
export function TimesheetReports() {
    ...
    const [selectedStatusName, setSelectedStatusName] = useState<string>('all');
    ...
    useEffect(() => {
        if (reportsData && reportsData.results) {
            const transformedData: TimesheetRecord[] = reportsData.results.map((report: any, index: number) => ({
                ...
                user_country_code: report.user_country_code,
    ...
    const applyFilters = useCallback(() => {
        ...
        if (selectedStatusName !== 'all') {
            newFilters.status_name = selectedStatusName;
        }
        ...
        getDateRangeForAPI,
        selectedUserDepartment, selectedProjectDepartment, selectedCountryCode,
        selectedUserCountryCode, selectedDetailedGrade, selectedStatusName,
    ...
    const handleExportExcel = () => {
        ...
        if (selectedUserCountryCode !== 'all') exportParams.user_country_code = selectedUserCountryCode;
        if (selectedDetailedGrade !== 'all') exportParams.detailed_grade = selectedDetailedGrade;
        if (selectedStatusName !== 'all') exportParams.status_name = selectedStatusName;
            ...
            <Card>
                ...
                <CardContent>
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 mb-4">
                        ...
                        <div>
                            <Select value={selectedStatusName} onValueChange={setSelectedStatusName}>
                                <SelectTrigger className="h-9 text-sm">
                                    <SelectValue placeholder="Status" />
                                </SelectTrigger>
                                <SelectContent>
                                    <SelectItem value="all">User Statuses</SelectItem>
                                    {accounts_statuses && accounts_statuses.map((status: AccountStatus) => (
                                        <SelectItem key={status.id} value={status.name || status.status || ''}>
                    ...
                    {showCustomDates && (
                        <div className="mb-4 mt-2" key="custom-dates-container">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3" style={{ display: 'flex' }}>
                                <div>
                                    ...
                                    <TableBody>
                                        {timesheetData.map((record) => (
                                            <TableRow key={record.id} className="text-sm">
                                                ...
                                                <TableCell>
                                                    <span className={`px-1.5 py-0.5 rounded text-xs ${record.status_name === 'active' ? 'bg-green-100 text-green-800' :
                                                        record.status_name === 'inactive' ? 'bg-red-100 text-red-800' :
